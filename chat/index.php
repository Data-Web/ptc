...
<body>
<body background="imgs/blogk.net-linen.jpg">
<?
switch($_GET['param']):
...
<div id="wrapper">
	<div id="nav"> 
    	<div id="nav_link">
<a href="http://raovatlangson.vn">Trang Chủ</a>
<?
        if(!$_SESSION['user_login']) {
            echo '<a href="./">Phòng Chat</a>';
			echo '<a href="javascript:ajax_load(\'?param=sign&act=up\',\'main\')">Đăng ký</a>';
            echo '<a href="javascript:ajax_load(\'?param=sign&act=in\',\'main\')">Đăng nhập</a>';}
?>
<? if($_SESSION['user_login']) 
			echo '<a href="?param=logout">Thoát</a>';
		?>

        </div>

        <div id="welcome">
<MARQUEE width="150" scroll="continuous" behavior="scroll" scrollamount="5" scrolldelay="5" onmouseover="this.stop();" onmouseout="this.start();" direction="left" style="margin-top:5px; color:#FFF;">
        <? if($_SESSION['user_login']) 
    ...
    <div id="shoutform">
        ...
        <div id="upstyle">
			...
			<? if(in_array($_SESSION['user_login'],$config['admin'])){echo '<input type="button" class="sbutton" value="Clear" onclick="javascript:send_chat1();" //>' ;}?>
			<? if(in_array($_SESSION['user_login'],$config['admin'])){echo '<a href="onoff.php">Bật | Tắt</a>' ;}?>
        </div> <!--/#upstyle-->
        <div id="upwrite">
        	<div style="float:left"><input maxlength="255" type="text" id="uptext"/></div>
            <div class="Send"><input type="image" name="btn" class="Send-Button"/></div>
